...
async function loadData() {
  ...
  allSchools = [
    ...processIPSData(ipsEcoles, 'Ã©cole', 'ips', locMap, effMap),
    ...processIPSData(ipsColleges, 'collÃ¨ge', 'ips', locMap, effMap),
    ...processIPSData(ipsLycees, 'lycÃ©e', 'ips_etab', locMap, effMap)
  ];
}
...
function applyFilters() {
  filteredSchools = allSchools.filter(school => {
    ...
    if(type.includes('Ã©cole') && !currentFilters.types.ecoles) return false;
    if(type.includes('collÃ¨ge') && !currentFilters.types.colleges) return false;
    if(type.includes('lycÃ©e')) {
    ...
    if(sector.includes('privÃ©') && !currentFilters.sectors.private) return false;
  ...
  updateMarkers();
  updateStatistics();
  // suppression du dÃ©zoom automatique au filtre
}
...
function updateMarkers() {
  ...
  filteredSchools.forEach(school => {
    ...
    } else if (type.includes('collÃ¨ge')) {
...
function createDetailedPopup(school) {
  ...
  html += `<div class="popup-info">${escapeHtml(school.type)} â€¢ ${escapeHtml(school.sector)}</div>`;
  ...
  if (school.nombre_total_eleves !== null && school.nombre_total_classes !== null) {
    html += `<div class="popup-compact-row"><span>ðŸ‘¥ Ã‰lÃ¨ves | Classes :</span> <span>${school.nombre_total_eleves} | ${school.nombre_total_classes}</span></div>`;
  }
  return html;
}
...
function updateStatistics() {
  ...
  let privCount = filteredSchools.filter(s => s.sector.toLowerCase().includes('privÃ©')).length;
...
function zoomToFiltered() {
  // volontairement vide pour ne pas dÃ©zoomer Ã  chaque filtre
}
...
function updateDepartmentFilter() {
  ...
  departmentSelect.innerHTML = '<option value="">Tous les dÃ©partements</option>';
