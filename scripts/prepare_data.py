...
Points cles de cette version :
  ...
  - `historique_resultats.json` (nouveau, meme principe de chargement a la
    demande) contient l'historique IVAC (colleges) / IVAL voie GT (lycees) au
    format compact {code_uai: [[annee, taux_reussite, va_taux_reussite], ...]}.
    Les deux fichiers sources (IVAC colleges, IVAL lycees GT) sont deja
    multi-annees (colonnes session/annee), exactement comme l'IPS — seule
    l'IVAL voie pro n'a pas d'historique ici (source a millesime unique, pas
    de colonne annee permettant d'empiler plusieurs sessions).

Sorties : public/data/identite.json, public/data/indicateurs.json,
          public/data/resultats.json, public/data/historique_ips.json,
          public/data/historique_resultats.json
"""
...
def _ival_lycees_gt_empile():
    return dedupe_par_uai_annee(
        charger_csv_multi_annees(SOURCE_IVAL_LYCEES_GT, colonnes_annee=["annee"])
    )


def construire_historique_resultats():
    historique = {}
    ...
    for uai, lignes_uai in par_uai.items():
        ...
        pts = [
            [l["annee"], to_float(l.get("taux_de_reussite_g")), to_float(l.get("va_du_taux_de_reussite_g"))]
            for l in lignes_uai
            if to_float(l.get("taux_de_reussite_g")) is not None
        ]
        if len(pts) >= 2:
            historique[uai] = pts

    lignes_ival_gt = _ival_lycees_gt_empile()
    par_uai = defaultdict(list)
    for l in lignes_ival_gt:
        par_uai[l["uai"]].append(l)
    for uai, lignes_uai in par_uai.items():
        lignes_uai.sort(key=lambda l: l["annee"])
        pts = [
            [l["annee"], to_float(l.get("taux_reu_total")), to_float(l.get("va_reu_total"))]
            for l in lignes_uai
            if to_float(l.get("taux_reu_total")) is not None
        ]
        if len(pts) >= 2:
            historique[uai] = pts
...
def main():
    ...
    print("\n--- Construction historique résultats (IVAC/IVAL multi-années) ---")
    historique_resultats = construire_historique_resultats()
    uai_indicateurs_pour_historique = {e["code_uai"] for e in indicateurs}
    historique_resultats_final = {
        uai: pts for uai, pts in historique_resultats.items() if uai in uai_indicateurs_pour_historique
    }
    ...
    with open(OUT / "historique_resultats.json", "w", encoding="utf-8") as f:
        json.dump(historique_resultats_final, f, ensure_ascii=False, separators=(",", ":"))
    ...
    print(f"historique_resultats.json : {len(historique_resultats_final)} établissements avec historique IVAC/IVAL")
